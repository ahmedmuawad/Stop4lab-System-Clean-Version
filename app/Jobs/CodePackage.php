...
use Carbon\Carbon;
class CodePackage implements ShouldQueue
{
    ...
    public function handle()
    {
        foreach($this->tests as $test){
            ...
            $pakage = Package::first();
            $gpc = 'EG-'.$this->portalSetting['registerationNumber'].'-'.$pakage->id.'package';
            $addProduct = '{
                "items": [
                    {
                        "codeType": "' . "EGS" . '",
            ...
            if ($product["passedItemsCount"] > 0) {
                return redirect()->route('admin.portal.packages')->with('success', "تم ارسال المنتج (قيد الإنتظار)" . ' ' . ($product['passedItems'][0]['itemCode']));
