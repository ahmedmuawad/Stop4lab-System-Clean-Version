...
class AjaxController extends Controller
{
    ...
    public function get_patient_by_code(Request $request)
    {
        ...
        if (isset($request->term)) {
            ...
            $patients = Patient::with('contract.sub_contracts')
    ...
    public function patient_exist(Request $request)
    {
    ...
    public function getPrefix(){
        $prefixies = Prefix::all();
        return response()->json($prefixies);
    }

    public function docter_exist(Request $request)
    {
        if (isset($request->name)) {
            Log::Info(['name' => $request->name]);
            $doctors = Doctor::where('name', $request->name)
                ->get();
        } else {
            $doctors = [];
        }
        Log::Info(['doctor' => $doctors]);
    ...
    {
        // dd($request->all())
        $patient = Patient::with('contract.sub_contracts', 'contract.tests', 'contract.cultures', 'contract.packages', 'country')->find($request->id);

        $age = explode(' ', $patient['age']);

        $patient['age_splited'] = $this->get_age($patient['dob']);

        // ['age' => $age[0] , 'age_unit' => strtolower($age[1]) ];
    ...
    {
        if (isset($request->term)) {
            $tests = Test::where('name', 'like', '%' . $request->term . '%')->where('parent_id', 0)->orWhere('separated', true)
                ->take(20)->get();
        } else {
    ...
    public function get_contract(Request $request, $id)
    {
        ...
        $contract = Contract::with(
            ...
            ['cultures' => function ($query) use ($request) {
     ...
     * @var  @Request $request
     */
    public function get_income_chart2()
    {
        ...
        for ($i = 1; $i < 32; $i++) {
            ...
            $incomeDate[] = Group::where('branch_id', session('branch_id'))->whereDate('created_at', date('Y-m-d', strtotime($year . '-' . $month . '-' . $i)))->sum('total');
            $expenDate[] = Expense::where('branch_id', session('branch_id'))->whereDate('date', date('Y-m-d', strtotime($year . '-' . $month . '-' . $i)))->sum('amount');
    ...
    public function get_tax_select2(Request $request)
    {
        if (isset($request->term)) {
            $taxes = TaxType::where('name_ar', 'like', '%' . $request->term . '%')->orwhere('name_en', 'like', '%' . $request->term . '%')->get();
        } else {
            $taxes = TaxType::get();
        }
    ...
    public function get_age($dob)
    {
        ...
        if ($interval->y == 0) {
            if ($interval->m == 0) {
                return response()->json(['interval' => $interval, 'age' => $interval->d, "unit" => "days"]);
            } else {
                return response()->json(['interval' => $interval, 'age' => $interval->m, "unit" => "months", "days" => $interval->d]);
            }
        } else {
            return response()->json(['interval' => $interval, 'age' => $interval->y, "unit" => "years", "months" => $interval->m]);
        }
    }
    /**
     * get dob from age
    ...
    {
        if (isset($request->term)) {
            $categies = Category::where('name', 'like', '%' . $request->term . '%')->take(20)->get();
        } else {
            $categies = Category::take(20)->get();
        }

        return response()->json($categies);
    }
    /* ++++++++++++++++++++++ get invoice tests ++++++++++++++++++++++ */
    public function get_invoice_tests(Request $request)
    {
        // if write "anyThing" in "search inputField"
        if (isset($request->term)) {
            $tests = Test::where(function ($query) {
                return $query->where('parent_id', 0)->orWhere('separated', 1);
            })
                ->where('name', 'like', '%' . $request->term . '%')
                ->orWhere('id', 'like', '%' . $request->term . '%')->orwhere('shortcut', 'like', '%' . $request->term . '%')
                ->take(50)
                ->with('category')
                ->get();
        } else {
    ...
    public function get_invoice_offers(Request $request)
    {
        if (isset($request->term)) {
            $offers = Offers::where('name', 'like', '%' . $request->term . '%')
                ->orwhere('shortcut', 'like', '%' . $request->term . '%')
                ->orwhere('cost_afetr', 'like', '%' . $request->term . '%')
                ->orwhere('cost_before', 'like', '%' . $request->term . '%')
                ->where('status', 'active')
                ->get();
        } else {
    ...
    public function get_invoice_offer(Request $request)
    {
        $offer = Offers::where('id', $request['offer_id'])->first();
        return response()->json($offer);
    }

    public function get_invoice_extraServices(Request $request)
    {
        if (isset($request->term)) {
            $extraServices = ExtraServices::where('name', 'like', '%' . $request->term . '%')
                ->orWhere('id', 'like', '%' . $request->term . '%')->get();
        } else {
            $extraServices = ExtraServices::get();
        }
        return response()->json($extraServices);
    }

    public function get_invoice_extraService(Request $request)
    {

        $service = ExtraServices::find($request['service_id']);
    ...
    {
        $test = Test::with('category', 'questions', 'sample' , 'test_price')->find($request['test_id']);
        ...
        if (!empty($request['contract_id'])) {
            ...
            if ($contract != null) {
                ...
                $test['is_refuse'] = $contract['is_refuse'];
            } else {
    ...
    {
        $culture = Culture::with('category', 'culture_price')->find($request['culture_id']);
        if (!empty($request['contract_id'])) {
            $contract = ContractPrice::where([
                ['contract_id', $request['contract_id']],
            ...
            $culture['is_refuse'] = $contract['is_refuse'];
        } else {
    ...
    {
        $package = Package::with('category', 'tests.test', 'package_price')->find($request['package_id']);
        if (!empty($request['contract_id'])) {
            $contract = ContractPrice::where([
                ['contract_id', $request['contract_id']],
            ...
            $package['is_refuse'] = $contract['is_refuse'];
        } else {
            if (!setting('medical')['samePrice']) {
    ...
    public function printer(Request $request)
    {
        ...
        $printer = new Printer();
        $printer->url_pdf = $request->pdf;
        $printer->print_type = $request->type;
        $printer->printer_name = setting('printer')["$request->type"];
        $printer->save();

        return $printer;
    }

    public function get_branches_report(Request $request)
    {
        ...
        foreach ($user->branches as $branche) {
            array_push($branchIds, $branche->branch_id);
        }

        if (isset($request->term)) {
            $branches = Branch::whereIn('id', $branchIds)->where('name', 'like', '%' . $request->term . '%')->take(20)->get();
        } else {
            $branches = Branch::whereIn('id', $branchIds)->take(20)->get();
        }
        return response()->json($branches);
    }

    public function get_sub_contract(Request $request)
    {
        if (isset($request->term)) {
            $patients = Contract::with('sub_contracts')
                ->where('title', 'like', '%' . $request->term . '%')
                ->take(20)
                ->get();
        } else {
            $patients = Contract::where('id', $request->id)->get();
        }

        return response()->json($patients);
    }
    public function get_rays(Request $request)
    {
        if (isset($request->term)) {
            $rays = Ray::where('name', 'like', '%' . $request->term . '%')
                ->take(20)
    ...
    {
        $request = EmployeeRequest::where('status', null)->get();

        return response()->json($request);
    }

    public function set_test_receive(Request $request)
    {
        if($request->testStatus == 'test'){
            GroupTest::where('id',$request->strID)->update(['is_receive' => $request->strState]);
        }
        return response()->json($request);

    }
    public function get_crystals_data(Request $request)
    {
        // if($request->reactionResult == 'Acidic'){
            $data = [
                [
                    'id' => 'Absent',
                    'text' => 'Absent'
