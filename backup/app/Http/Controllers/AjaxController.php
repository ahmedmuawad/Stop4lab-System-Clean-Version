...
class AjaxController extends Controller
{
    ...
    */
    public function get_income_chart($month,$year,Request $request)
    {
        ...
        for($i=1;$i<32;$i++)
        {
            $payment_amount=(empty($request['branch_id']))?GroupPayment::whereDate('date',date('Y-m-d',strtotime($year.'-'.$month.'-'.$i)))->sum('amount'):GroupPayment::whereHas('group',function($q)use($request){$q->where('branch_id',$request['branch_id']);})->whereDate('date',date('Y-m-d',strtotime($year.'-'.$month.'-'.$i)))->sum('amount');
