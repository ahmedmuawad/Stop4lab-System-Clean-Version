<div class="row">
    ...
    <div class="col-lg-3 lab_out @if (isset($test)) @if ($test['lab_to_lab_status'] == 0) d-none @else d-block  @endif @else d-none  @endif">
        <div class="form-group">
                ...
                <select name="lab_out_id" class="form-control select2">
                    ...
                    @if(isset($labs_out))
                        @foreach ($labs_out as $lab)
                            <option @if(isset($test) && $test->lab_out_id == $lab->id) selected @endif  value="{{ $lab->id }}">{{ $lab->name }}</option>
                        @endforeach
                    @endif
                </select>
            </div>
    ...
    <div class="col-lg-6">
        <div class="form-group">
             ...
             <textarea name="decreased_in" id="Decreased" rows="3" class="form-control" placeholder="{{__('Decreased In')}}">@if(isset($test)){{$test['decreased_in']}}@endif</textarea>
        </div>
    </div>
    <div class="col-lg-6">
        <div class="form-group">
             <label for="Increased">{{__('Increased In')}}</label>
             <textarea name="increased_in" id="Increased" rows="3" class="form-control" placeholder="{{__('Increased In')}}">@if(isset($test)){{$test['increased_in']}}@endif</textarea>
        </div>
    </div>
                                ...
                                @if(isset($test))
                                    @foreach($test['components'] as $component)
                                        ...
                                        <tr num="{{$count}}" test_id="{{$component['id']}}">
                                            ...
                                            @else
                                                ...
                                                <td style="vertical-align: top;">
                                                    <ul class="p-0 list-style-none">
                                                        ...
                                                        <li>
                                                            <input class="select_type" value="multy" type="radio" name="component[{{$count}}][type]" id="select_{{$count}}" @if($component['type']=='multy') checked @endif required> <label for="select_{{$count}}">{{__('Multiple')}}</label>
                                                        </li>
                                                    </ul>
                                                    <div class="options">
                                                        ...
                                                        @elseif($component['type']=='multy')
                                                            ...
                                                            <table width="100%">
                                                                ...
                                                                @foreach($component['options_additional'] as $option)
                                                                    <tr option_id="{{$option['id']}}">
                                                                        <td>
                                                                            <input type="text" name="component[{{$count}}][old_options_additional][{{$option['id']}}]" value="{{$option['name']}}" class="form-control" required>
                                                                        </td>
                                                                        <td>
                                                                            <button type="button" class="btn btn-danger btn-sm text-center delete_option">
                                                                                <i class="fa fa-trash"></i>
                                                ...
                                                <td colspan="7">
                                                    <table class="table table-bordered reference_range"
                                                        <thead>
                                                            <tr>
                                                              <th class="gender text-center" style="width: 18%">{{__('Gender')}}</th>
                                                              <th class="age_unit text-center" style="width: 18%">{{__('Age unit')}}</th>
                                                              <th class="age_from text-center" style="width: 13%">{{__('Age')}}</th>
                                                        ...
                                                        <tbody>
                                                            @foreach($component['reference_ranges'] as $reference_range)
                                                                ...
                                                                <tr>
                                                                    <td colspan="6"><textarea name="component[{{$count}}][reference_ranges][{{$count_reference_ranges}}][comment]" style="width: 100%" rows="2">{{ $reference_range->comment }}</textarea></td>
                                                                </tr>
                                                            @endforeach
                                                        </tbody>
                                                    </table>
                                            ...
                                            <tr>
                                                <td class="text-center">
                                                    <label>Separated </label><input class="check_separated" num="{{$count}}" type="checkbox" name="component[{{$count}}][separated]" @if($component['separated']) checked @endif>
                                                ...
                                                <td class="text-center">
                                                    <label>Status </label> <input  type="checkbox" name="component[{{$count}}][status]" @if($component['status']) checked @endif>
                                                </td>
                                                <td class="text-center">
                                                    <input type="number"  name="component[{{$count}}][min]" value="{{isset($component) ? $component->min : 0 }}" required>
                                                </td>
                                                <td class="text-center">
                                                    <input type="number"  name="component[{{$count}}][max]" value="{{isset($component) ? $component->max : 1000 }}" required>
                                                </td>
                                                <td>
