    ...
    @foreach ($branches as $branch)
        @if (count($branch->works))
            <table class="table" width="100%">
                <thead>
                    <tr class="title">
                        <th colspan="{{ count($branch->works) + 1 }}">
                            {{ $branch->name }}
                        </th>
                    </tr>
                    @if (count($branch->works))
                ...
                <tbody>
                    @if (count($branch->works))
                        ...
                        <tr>
                            <td>{{ __('Total Paid') }} </td>
                            @foreach ($branch->works as $work)
                                <td>{{ $work['totalPaid'] }} </td>
                            @endforeach
                        </tr>
                        <tr>
                            <td>{{ __('Custody Of Branch') }}</td>
                            @foreach ($branch->works as $work)
                                <td>{{ $work['branchCustody'] }} </td>
                            @endforeach
                        </tr>
                        <tr>
...
{{-- {{ dd($work->paid) }} --}}
                        ...
                        @endforeach --}}
{{-- 
                        @for ($i = 0 ; $i < count($branch->works[0]->paid) ; $i++)
                            <tr>
                                @foreach ($branch->works as $work)
                                    <tr>
                                        @foreach ($work->paid as $key => $payment_method)
                                            <td>{{ $payment_method->name }} </td>
                                            <td>{{ $payment_method->income }} </td>
                                        @endforeach
                                    </tr>
                                    
                                    {{-- <td>{{ $work['paid'][$i]['name'] }} </td> 
                                @endforeach
                            </tr>
                         @endfor--}}
                        <tr>
