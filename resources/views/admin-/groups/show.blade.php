...
@section('content')
    <div class="card card-primary">
        ...
        <input type="hidden" name="patient_code"
            @if (isset($group['patient'])) value="{{ $group['patient']['code'] }}" @endif id="patient_code">
            ...
            <div class="p-3 mb-3" id="invoice">
                                                ...
                                                <tr>
                                                    ...
                                                    <td width="50%">
                                                        <img src="data:image/png;base64,{{ DNS1D::getBarcodePNG($group['barcode'], $barcode_settings['type'][session('branch_id')]) }}"
                                                            alt="barcode" class="margin" width="100" /><br>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td width="50%">
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        ...
                                                        <span class="data">
                                                            ...
                                                            {{ $group['resulte_date'] == null ? receivedDate($group['id']) : $group['resulte_date'] }}
                                                        </span>
                                                    </td>
                ...
                <div class="row">
                    ...
                    <div class="col-lg-12 table-responsive">
                        <p class="lead">{{ __('extraService') }}</p>
                            ...
                            <table class="table table-bordered">
                                ...
                                @foreach ($extraService as $item)
                                    <tr>
                                        ...
                                        <td colspan="2">
                                            {{ $item->extraService->descript }}
                                        </td>

                                        <td>
                                            {{ formated_price($item->extraService->price) }}
                                            @php
        ...
        <div class="card-footer">
            ...
            <div class="row no-print">
                ...
                <div class="col-12">
                    ...
                    <a target="_blank" href="{{ url('uploads/pdf_/receipt_2_' . $group['id'] . '.pdf') }}"
                        class="btn btn-danger">
                        <i class="fa fa-file-pdf"></i> {{ __('Print receipt') }} A 4
                    </a>
                    {{--  <a target="_blank" href="{{$group['receipt_pdf']}}" class="btn btn-danger">
                    <i class="fa fa-file-pdf"></i> {{__('Print receipt Direct')}}
                </a>  --}}

                    <a style="cursor: pointer" class="btn btn-warning print_barcode" data-toggle="modal"
                        data-target="#print_barcode_modal{{ $group['id'] }}" group_id="{{ $group['id'] }}">
                        <i class="fa fa-barcode" aria-hidden="true"></i>
                        {{ __('Print_barcode_file') }}
                    </a>
                    <a style="cursor: pointer" class="btn btn-warning Print_barcode_file" data-toggle="modal"
                        data-target="#print_barcode_file_modal{{ $group['id'] }}" group_id="{{ $group['id'] }}">
                        <i class="fa fa-barcode" aria-hidden="true"></i>
                        {{ __('Print barcode') }}
                    </a>
                    ...
                    @if ($whatsapp['receipt']['active'] && isset($group['receipt_pdf']))
                        ...
                        @endphp
                        <a target="_blank"
                            href="https://wa.me/{{ $group['patient']['phone'] }}?text={{ $message }}"
                    ...
                    @if ($email['receipt']['active'] && isset($group['receipt_pdf']))
                        <form action="{{ route('admin.groups.send_receipt_mail', $group['id']) }}" method="POST"
                            class="d-inline">
                            @csrf
                    ...
                    @can('edit_medical_report')
                        <a href="#" data-toggle="modal" class="btn btn-primary d-inline"
                    ...
                    @can('edit_patient')
                        <a href="{{ route('admin.patients.edit', $group['patient']['id']) }}"
                            class="btn btn-warning btn-sm mr-1 float-right">
                            <i class="fa fa-check"></i>
                            {{ __('Edit patient') }}
                        </a>
                    @endcan
                    ...
                    @if ($group->rays->count())
                        <a href="#" data-toggle="modal" class="btn btn-primary btn-sm mr-1 float-left"
                            data-target="#exampleModalCenterBase{{ $group['id'] }}">
                            <i class="fa fa-check"></i>
                            {{ __('Select Based for Rays') }}
                        </a>
                    @endif
        ...
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.groups.print_barcode_file', $group['id']) }}" method="POST"
                    id="print_barcode_file_form" target="_blank">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-lg-12">
    ...
    <div class="modal fade" id="exampleModalCenterBase{{ $group['id'] }}" tabindex="-1" role="dialog"
        aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document" style="max-width:1050px !important;">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <form action="{{ route('admin.group.ray.based_by', $group['id']) }}" method="post">
                        @csrf()
                        @foreach ($group->rays as $ray)
                            <div class="form-group">
                                <div class="row">
                                    <div class="col-6">
                                        <label for="">{{ $ray->ray->name }}</label>

                                    </div>
                                    <div class="col-6">
                                        <label for="">{{ __('Based By') }}</label>
                                        <select name="based_by[{{ $ray->ray->id }}]" id=""
                                            class="form-control based_by select2">

                                            @if ($ray->base_by != null)
                                                <option value="{{ $ray->base_by->id }}" selected>
                                                    {{ $ray->base_by->name }}</option>
                                            @endif
