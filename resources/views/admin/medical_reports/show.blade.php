...
@section('content')
            ...
            <div class="row mb-3">
                ...
                <div class="col-lg-9">
                    ...
                    <button type="button" id="withoutHeaders"
                        data-action="{{ route('admin.medical_reports.print_report', $group['id']) }}"
                        ...
                        <div class="row">
                            ...
                            <div class="n-chk">
                                <label class="new-control new-radio radio-primary">
                                    <input type="radio" class="new-control-input report_design" name="report_design"
                                        value="1"checked>
                                    ...
                                    <table width="100%">
                                        <tbody id="analysis_titles_sort">
                                            ...
                                            @php
                                                ...
                                                $endid = 0;
                                            @endphp
                                            @foreach ($categories->sortBy(function ($cate) { if($cate->tests->count() > 0 ){ return $cate->tests[0]->test->components->count();  }else{ return 0; }  }    ); as $category)
                                                @foreach ($category['tests'] as $test)                                                   
                                                    @php
                                                        $categoryIds[$loop->iteration] = $test->test->category_id;
                                                        $index = ($loop->iteration > 1 )? $loop->iteration - 1 : $loop->iteration; 
                                                    @endphp
                                                    <tr>
                                                        <td>
                                                            <div class="card card-primary card-outline collapsed-card"
                                                                ...
                                                                <div class="card-header">
                                                                    <h4 class="card-title">
                                                                        ...
                                                                        @if (!$group['uploaded_report'])
                                                                            <input type="checkbox"
                                                                                class="get_tests analyses_select"
                                                                    ...
                                                                    @if ($endid > 0 && $test->test->category_id != $endid)
                                                                        ...
                                                                        <input type="checkbox" class="newline"
                                                                            ...
                                                                            id="new_line[{{ $test['id'] }}]" value="1"
                                                                            
                                                                            @if ( (isset($test->setting['new_line']) &&  $test->setting['new_line'])  || $test['new_line'] == '1') checked @endif>
                                                                    @endif
                                                                    

                                                                    @if($test->test->category_id != $categoryIds[$index] || $loop->iteration == 1 )

                                                                        <a style="cursor: pointer" data-toggle="modal" data-target="#setting_modal{{$test['id']}}" class="dropdown-item print_barcode" group_id="{{$group['id']}}">
                                                                ...
                                                                <div class="card-body p-0">
                                                                    ...
                                                                    @if (in_array($test->test->id,setting('medical')['cbc']))
                                                                        <x-cbc-show-component :test="$test" :group="$group" />
                                                                    @else
                                                                        <table
                                                    ...
                                                    <div>
                                                        ...
                                                        @if ($loop->iteration > 1)
                                                            ...
                                                            <input type="checkbox" class="newline"
                                                                ...
                                                                id="new_line[{{ $culture['id'] }}]" value="1"
    ...
    @foreach ($group['all_tests']->sortBy(function ($test) {return $test->test->category_id;}) as $test)
        ...
        <form action="{{ route('admin.medical_report.setting_test', $test['id']) }}" method="POST" id="setting_test_{{ $test->id }}">
            @csrf
            @php
                $settings = json_decode($test['setting'], true);
            @endphp
            <x-setting-component :test="$test" :settings="$settings" />
        </form>
    @endforeach
