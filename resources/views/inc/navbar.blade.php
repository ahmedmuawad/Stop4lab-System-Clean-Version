            ...
            @if (isset($employee))
                <li>
                    <button class="btn btn-info btn-sm  ml-1" type="button" id="">
                        <a class="" onclick="document.getElementById('cheackin_form').submit();"><i class="mr-50"
                                data-feather="user"></i>{{ __('Check In') }} </a>
                    </button>
                    <button class="btn btn-info btn-sm  ml-1" type="button" id="">
                        <a onclick="document.getElementById('cheackout_form').submit();"><i class="mr-50"
                                data-feather="user"></i>{{ __('Cheack Out') }} </a>
                    </button>
...
<ul class="navbar-item flex-row ml-md-auto">
        ...
        <div class="dropdown-menu position-absolute" aria-labelledby="language-dropdown">
            @foreach ($languages as $lang)
                @if (app()->getLocale() != $lang['iso'])
                    <a href="{{ route('change_locale', $lang['iso']) }}" class="dropdown-item d-flex"
                        ...
                        @if ($lang['iso'] != 1)
                            ...
                            </svg><span class="align-self-center">{{ $lang['iso'] }}</span>
                    </a>
                @else
                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="64px"
                        height="64px" viewBox="0 0 64 64" aria-hidden="true" role="img"
                        ...
                        <path d="M15.9 57.3h32.2c2.1-1.3 3.9-2.9 5.6-4.6H10.3c1.7 1.8 3.6 3.3 5.6 4.6"
                            fill="#fff" />
                    ...
                    </svg><span class="align-self-center">{{ $lang['iso'] }}</span></a>
                @endif
    ...
    <li class="nav-item dropdown user-profile-dropdown language-dropdown">
        ...
        <a href="javascript:void(0);" class="nav-link dropdown-toggle user" id="userProfileDropdown"
            ...
            @if (auth()->guard('admin')->check())
                <img src="@if (
                    !empty(auth()->guard('admin')->user()->avatar
                    )) {{ url('uploads/user-avatar/' .auth()->guard('admin')->user()->avatar) }}@else {{ url('img/avatar.png') }} @endif"
                    alt="avatar">
            @else
                <img src="@if (
                    !empty(auth()->guard('patient')->user()->avatar
                    )) {{ url('uploads/patient-avatar/' .auth()->guard('patient')->user()->avatar) }}@else {{ url('img/avatar.png') }} @endif"
                    alt="avatar">
            @endif
        ...
        <div class="dropdown-menu position-absolute" aria-labelledby="userProfileDropdown">
            <div class="">
                <div class="dropdown-item">
                    <a class="" href="{{ route('admin.profile.edit') }}"><svg
...
<div class="sub-header-container">
    <header class="header navbar navbar-expand-sm"
        style="@media (min-width: 768px){.sub-header-container { margin-top: 40px;}}">
        {{--  <a href="javascript:void(0);" class="sidebarCollapse" data-placement="bottom">
            <svg
    ...
    <header class="header navbar navbar-expand-sm">
        <a href="javascript:void(0);" class="sidebarCollapse" data-placement="bottom">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"
                ...
                style="color: white; margin-right: 30px; margin-left: 30px;}">
                <line x1="3" y1="12" x2="21" y2="12"></line>
                <line x1="3" y1="6" x2="21" y2="6"></line>
                <line x1="3" y1="18" x2="21" y2="18"></line>
            </svg></a>
        <ul class="navbar-item theme-brand flex-row  text-center">
            ...
            <li class="nav-item theme-text">
                <a href="#" class="nav-link"> {{ setting('info')['name']  }} </a>
            </li>
        </ul>
        ...
        <ul class="navbar-item flex-row ml-md-auto">
            ...
            <li class="nav-item dropdown language-dropdown">
                ...
                <div class="dropdown-menu position-absolute" aria-labelledby="language-dropdown">
                    @foreach ($languages as $lang)
                        ...
                        @else
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                width="64px" height="64px" viewBox="0 0 64 64" aria-hidden="true" role="img"
                                class="iconify iconify--emojione" preserveAspectRatio="xMidYMid meet">
                                <path d="M48 6.6C43.3 3.7 37.9 2 32 2v4.6h16" fill="#ed4c5c" />
                                <path d="M32 11.2h21.6C51.9 9.5 50 7.9 48 6.6H32v4.6z" fill="#fff" />
                                <path d="M32 15.8h25.3c-1.1-1.7-2.3-3.2-3.6-4.6H32v4.6z" fill="#ed4c5c" />
                                ...
                                <path d="M61.9 29.7H32V32H2c0 .8 0 1.5.1 2.3h59.8c.1-.8.1-1.5.1-2.3c0-.8 0-1.6-.1-2.3"
                                    fill="#ed4c5c" />
                                <path d="M2.8 38.9h58.4c.4-1.5.6-3 .7-4.6H2.1c.1 1.5.3 3.1.7 4.6" fill="#fff" />
                                <path d="M4.3 43.5h55.4c.6-1.5 1.1-3 1.5-4.6H2.8c.4 1.6.9 3.1 1.5 4.6"
                                    fill="#ed4c5c" />
                                <path d="M6.7 48.1h50.6c.9-1.5 1.7-3 2.4-4.6H4.3c.7 1.6 1.5 3.1 2.4 4.6"
                                    fill="#fff" />
                                <path d="M10.3 52.7h43.4c1.3-1.4 2.6-3 3.6-4.6H6.7c1 1.7 2.3 3.2 3.6 4.6"
                                    fill="#ed4c5c" />
                                <path d="M15.9 57.3h32.2c2.1-1.3 3.9-2.9 5.6-4.6H10.3c1.7 1.8 3.6 3.3 5.6 4.6"
                                    fill="#fff" />
                                <path d="M32 62c5.9 0 11.4-1.7 16.1-4.7H15.9c4.7 3 10.2 4.7 16.1 4.7"
                                    fill="#ed4c5c" />
                                <path
                                    d="M16 6.6c-2.1 1.3-4 2.9-5.7 4.6c-1.4 1.4-2.6 3-3.6 4.6c-.9 1.5-1.8 3-2.4 4.6c-.6 1.5-1.1 3-1.5 4.6c-.4 1.5-.6 3-.7 4.6c-.1.8-.1 1.6-.1 2.4h30V2c-5.9 0-11.3 1.7-16 4.6"
                                    fill="#428bc1" />
                                <g fill="#fff">
                                    ...
                                    <path d="M9 27l.5 1.5H11l-1.2 1l.4 1.5l-1.2-.9l-1.2.9l.4-1.5l-1.2-1h1.5z" />
                                    <path
                                        d="M11.8 13l1.2-.9l1.2.9l-.5-1.5l1.2-1h-1.5L13 9l-.5 1.5h-1.4l1.2.9l-.5 1.6" />
                                    <path
                                        d="M3.8 25l1.2-.9l1.2.9l-.5-1.5l1.2-1H5.5L5 21l-.5 1.5h-1c0 .1-.1.2-.1.3l.8.6l-.4 1.6" />
                                </g>
                            </svg><span class="align-self-center">{{ $lang['iso'] }}</span></a>
            ...
            <li class="nav-item dropdown user-profile-dropdown">
                ...
                <a href="javascript:void(0);" class="nav-link dropdown-toggle user" id="userProfileDropdown"
                    ...
                    <img src="@if (
                        !empty(auth()->guard('patient')->user()->avatar
                        )) {{ url('uploads/patient-avatar/' .auth()->guard('patient')->user()->avatar) }}
                        @else {{ url('img/avatar.png') }} @endif"
                        alt="avatar">
                    {{--  @if (Auth::guard('admin')->check())
                    {{Auth::guard('admin')->user()->name}}
                    @else
