@extends('layouts.pdf_2')
@section('title')
    {{ __('Accounting report') }}-{{ date('Y-m-d') }}
@endsection
@section('content')
    <style>
        .title {
            font-size: 20px;
            background-color: #dddddd;
            border: 1px solid black !important;
            margin-bottom: 10px;
            font-family: 'arial';
        }

        .table {
            margin-top: 20px;
            font-family: 'arial';
        }

        .accounting_header {
            border: 2px solid black;
            background-color: #F0F0F0;
            font-family: 'arial';
        }

        th {
            font-family: 'arial';
        }
    </style>


        <div class="accounting_header">
            <h3 align="center">
                <img src="data:image/png;base64,"
                     width="15px" alt="">
                
            </h3>
            <h2> {{ $contract['title'] }} - {{ $info_settings['name'] }} </h2>

        </div>
        <!-- Report Details -->
        <table width="100%">
            <thead>
                <tr class="title">
                    <th colspan="6" align="center">
                        {{ $contract['title'] }} - {{ __('Tests Price List') }} - {{ get_system_date() }}
                    </th>
                </tr>
                <tr>
                    <th>{{ __('ID') }}</th>
                    <th>{{ __('Name') }}</th>
                    <th>{{ __('Category') }}</th>
                    <th>{{ __('Time') }}</th>
                    <th>{{ __('Price') }}</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($contract['tests'] as $test)
                    @if (isset($test['priceable']))
                        <tr>
                            <td>
                                {{ $test['priceable']['id'] }}
                            </td>
                            <td>
                                {{ $test['priceable']['name'] }}
                            </td>
                            <td>
                                {{ isset($test['priceable']['category']['name'])?$test['priceable']['category']['name'] : '' }}
                            </td>
                            <td>
                                {{ $test['priceable']['num_day_receive'] }} Days -
                                {{ $test['priceable']['num_hour_receive'] }} Hours
                            </td>
                            <td>
                                {{ $test['price'] }}
                            </td>
                        </tr>
                    @endif
                @endforeach
            </tbody>
        </table>
        <!-- \Report Details -->
        <!-- Report Details -->
        <table width="100%">
            <thead>
                <tr class="title">
                    <th colspan="6" align="center">
                        {{ $contract['title'] }} - {{ __('Culture Price List') }}  - {{ get_system_date() }}
                    </th>
                </tr>
                <tr>
                    <th>{{ __('ID') }}</th>
                    <th>{{ __('Name') }}</th>
                    <th>{{ __('Category') }}</th>
                    <th>{{ __('Time') }}</th>
                    <th>{{ __('Price') }}</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($contract['cultures'] as $test)
                    @if (isset($test['priceable']))
                        <tr>
                            <td>
                                {{ $test['priceable']['id'] }}
                            </td>
                            <td>
                                {{ $test['priceable']['name'] }}
                            </td>
                            <td>
                                {{ isset($test['priceable']['category']['name'])?$test['priceable']['category']['name'] : '' }}
                            </td>
                            <td>
                                {{ $test['priceable']['num_day_receive'] }} Days -
                                {{ $test['priceable']['num_hour_receive'] }} Hours
                            </td>
                            <td>
                                {{ $test['price'] }}
                            </td>
                        </tr>
                    @endif
                @endforeach
            </tbody>
        </table>
        <!-- \Report Details -->
        <!-- Report Details -->
        <table width="100%">
            <thead>
                <tr class="title">
                    <th colspan="6" align="center">
                        {{ $contract['title'] }} - {{ __('Packages Price List') }}  - {{ get_system_date() }}
                    </th>
                </tr>
                <tr>
                    <th>{{ __('ID') }}</th>
                    <th>{{ __('Name') }}</th>
                    <th>{{ __('Tests') }}</th>
                    <th>{{ __('Price') }}</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($contract['packages'] as $package)
                    @if (isset($package['priceable']))
                        <tr>
                            <td>
                                {{ $package['priceable']['id'] }}
                            </td>
                            <td>
                                {{ $package['priceable']['name'] }}
                            </td>

                            <td>
                                <ul>
                                    @foreach ($package->priceable->tests as $item)    
                                        <li>{{ (isset($item['test']['name']))? $item['test']['name'] : '' }}</li>
                                    @endforeach
                                    @foreach ($package->priceable->cultures as $item)    
                                        <li>{{ (isset($item['culture']['name']))? $item['culture']['name'] : '' }}</li>
                                    @endforeach
                                </ul>
                            </td>

                            <td>
                                {{ $package['price'] }}
                            </td>
                        </tr>
                    @endif
                @endforeach
            </tbody>
        </table>
        <!-- \Report Details -->



@endsection
