...
@section('content')
    ...
    <table class="table" width="100%">
        <thead>
               ...
               <tr>
                    <th align="center">{{ __('Test') }}</th>
                    <th align="center">{{ __('Count') }}</th>
                    <th align="center">{{ __('Value') }}</th>
               </tr>
        </thead>
        <tbody>
            @foreach ($groupTests as $test)
                <tr>
                    <td align="center">{{ $test->testName }}</td>
                    <td align="center">{{ $test->data }}</td>
                    <td align="center">{{ $test->price }} {{ get_currency() }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
            ...
            <table class="table table-bordered" width="100%">
               ...
               <tbody>
               <tr>
                   <th width="100px" align="left">{{ __('Total Count') }}:</th>
                   <td>{{ $groupTests->sum('data') }}</td>
               </tr>
               <tr>
                   <th width="100px" align="left">{{ __('Total Value') }}:</th>
                   <td>{{ formated_price($groupTests->sum('price')) }}</td>
               </tr>
               <tr>
                   <th width="100px" align="left">{{ __('Count Test') }}:</th>
                   <td>{{ count($groupTests) }}</td>
               </tr>
               </tbody>
           </table>
           <!-- \ Report Summary-->
