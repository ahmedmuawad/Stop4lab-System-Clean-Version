...
(function($){
    ...
    //cultures datatable
    table=$('#sample_types_table').DataTable( {
        ...
        "bSort" : false,
        "ajax": {
          url: url("admin/get_sample_types")
        },
        // orderCellsTop: true,
        fixedHeader: true,
        "columns": [
    ...
    $('.add_sample').on('click',function(){
       ...
       $('tbody').append(`
        <tr>
            <td>
                <input type="text" name="sub[`+count+`][name]" id="sub_`+count+`" placeholder="`+trans("sample name")+`"  class="form-control" required>
            </td>
            <td>
    ...
    //delete culture sample
    $(document).on('click','.delete_sample',function(e){
        e.preventDefault();
        var el=$(this);
        swal({
