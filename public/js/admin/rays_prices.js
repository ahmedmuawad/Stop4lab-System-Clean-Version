(function($){
    ...
    //datatable
    var rays_table=$('#rays_prices_table').DataTable( {
    ...
    $(document).on('change','.test_price',function(){
       ...
       if($('#hidden_prices').find('#test_price_'+test_id).length>0)
       {
            ...
            $('#hidden_prices').append(`
                <input type="hidden" class="hidden_price" test_id="`+test_id+`" name="rays[`+test_id+`]" value="`+price+`" id="test_price_`+test_id+`">
            `);
       }
    });

    rays_table.on( 'draw', function () {
        $('#hidden_prices .hidden_price').each(function(){
            var test_id=$(this).attr('test_id');
            var price=$(this).val();

            $('#rays_prices_table #test_'+test_id).val(price);
        });
    });

    var rays_table=$("#table_id").DataTable({
        "ordering": false,
