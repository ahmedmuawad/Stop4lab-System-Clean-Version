...
(function($){
    ...
    $('#discount_percentage').change(function(){
      ...
      tests_discounts.each(function(item){
        ...
        test_input.attr("value",Math.round(discount));
        item[2]=test_input.prop('outerHTML');
      });
      tests_table.clear().rows.add(tests_discounts).draw();
    ...
    if(discount_type==1)
    {
      ...
      tests_discounts.each(function(item){
        var test_input=$(item[2]);
        var price=parseFloat(test_input.attr('value'));
        //change html
        
        test_input.prop("readonly",true);
        test_input.attr("value",price);
        item[2]=test_input.prop('outerHTML');
      });
      tests_table.clear().rows.add(tests_discounts).draw();
    ...
    $('.add_contract').on('click',function(){
      ...
      $('.sub').append(`
       <tr>
           <td>
               <input type="text" name="sub[`+count+`][title]" id="sub_`+count+`" placeholder="`+trans("contract name")+`"  class="form-control" required>
           </td>
           <td>
               <input type="number" name="sub[`+count+`][discount_percentage]" max="100"  placeholder="`+trans("Discount Percentage")+`"  class="form-control" required>
           </td>
           <td>
