    ...
    var cultures_table=$('#cultures_prices_table').DataTable( {
        "lengthMenu": [[10, 25, 50,100,500,1000, -1], [10, 25, 50,100,500,1000 ,"All"]],
         ...
         "columns": [
            ...
            {data:"cost",sortable:false,orderable:false},
    ...
    $(document).on('change','.culture_price',function(){
       ...
       if($('#hidden_prices').find('#culture_price_'+culture_id).length>0)
       {
            ...
            $('#hidden_prices').append(`
                <input type="hidden" class="hidden_price" culture_id="`+culture_id+`" name="cultures[price][`+culture_id+`]" value="`+price+`" id="culture_price_`+culture_id+`">
            `);
       }
    });


    $(document).on('change','.culture_cost',function(){
        var cost=$(this).val();
        if(cost<0)
        {
            $(this).val(0);
        ...
        var culture_id=$(this).attr('culture_id');
        if($('#hidden_costs').find('#culture_cost_'+culture_id).length>0)
        {
             $('#hidden_costs').find('#culture_cost_'+culture_id).val(price);
        }
        else{
             $('#hidden_costs').append(`
                 <input type="hidden" class="hidden_cost" culture_id="`+culture_id+`" name="cultures[cost][`+culture_id+`]" value="`+cost+`" id="culture_cost_`+culture_id+`">
             `);
        }
     });
