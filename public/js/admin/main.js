...
function get_unread_messages()
{
  $.ajax({
    ...
    success:function(messages)
    {
      ...
      {
        // $(".message-dropdown").addClass('d-block').removeClass('d-none');
  ...
  $.ajax({
    ...
    success:function(visits)
    {
      ...
      if(visits.length>0)
      {
        ...
        $('.stock_alerts_count').text(visits.length);

        $('.list_stock_alerts').html(html);

          // $(".notification-dropdown").addClass('d-block').removeClass('d-none');

          var x = document.getElementById("sound-noty");

          x.play();
          swal({
            title: trans("New Visits"),
            type: "warning",
            showCancelButton: true,
            confirmButtonClass: trans("btn-success"),
        ...
        $('.stock_alerts_count').text('');
          // $(".notification-dropdown").removeClass('d-block').addClass('d-none');
      }
...
//                   <span class="badge badge-danger">`+stock_alerts.length+`</span> `+trans('Product alerts')+`
//                 </a>`;
...
function get_request_employee()
{

  $.ajax({
    url:ajax_url("get_request_employee"),
    success:function(request)
    {
      ...
      if(request.length>0)
      {
          ...
          html+=`<a href="`+url('admin/vocations')+`" class="dropdown-item">
                   <span class="badge badge-danger">`+request.length+`</span> `+trans('Request Employee alerts')+`
                </a>`;

          $('.stock_alerts_count').text(request.length);
...
// function get_new_booking()
// {

//   $.ajax({
//     url:ajax_url("get_new_booking"),
//     success:function(request)
//     {
//       var html=``;
...
//           html+=`<a href="`+url('admin/booking')+`" class="dropdown-item">
//                    <span class="badge badge-danger">`+request.length+`</span> `+trans('Request Employee alerts')+`
//                 </a>`;
...
function delete_stock_alerts_count()
{
  $stock = $('.stock_alerts_count');
...
//ajax url
function admin_ajax_url(url='')
{
    var base_url=location.origin;

    return base_url+'/new-design/ajax/'+url;
}
